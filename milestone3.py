...
def process_files(extracted_term):
    df1 = pd.read_csv("dropbox/liveBankAcct.csv")
    df1 = pd.DataFrame(df1)
    df2 = pd.read_csv("dropbox/liveCustomerList.csv")
    df2 = pd.DataFrame(df2)
    df3 = pd.read_csv(extracted_term)
    df3 = pd.DataFrame(df3)

    df3["Identity"] = df3["loginAcct"].astype(str) + "," + df3["custID"].astype(str)
    ...
    testdf = pd.merge(df3, df5_filter, on="Identity", how="left")
    ...
    testdf4.to_csv(extracted_term, index=False)
